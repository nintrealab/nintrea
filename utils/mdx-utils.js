...
export const EBOOK_PATH = path.join(process.cwd(), 'ebooks');
...
export const geteBooks = () => {
    let ebooks = postFilePaths.map((filePath) => {
        const source = fs.readFileSync(path.join(EBOOK_PATH, filePath));
        const { content, data } = matter(source);
